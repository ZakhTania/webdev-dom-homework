...
<div class="add-form-row">
  <button class="add-form-button">Написать</button>
    ...
    const linkWrapperElement = document.getElementById('wrapper-link');
    ...
    if (isLog) {
        linkWrapperElement.classList.add('hidden');
        addFormElement.classList.remove('hidden');
    } else {
        linkWrapperElement.classList.remove('hidden');
        addFormElement.classList.add('hidden');
    }

    modElement = isLog ? addFormElement : linkWrapperElement;

    fetchAndRenderComments({
        loader: loaderCommentFeedElement,
        waitingElement: modElement,
    });

    const authorizationLinkElement =
        document.getElementById('authorization-link');
    ...
    document.addEventListener('keyup', (event) => {
        if (event.code === 'Enter' && textInputElement.value.trim() !== '') {
            addComment();
        }
    });
