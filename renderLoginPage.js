...
export function renderLogin() {
    ...
    const loginHtml = `<div class="container">
                        <div class="authorization-form">
                            <h3 class="authorization-form_title">Форма ${reg ? `регистрации` : `входа`}</h3>
                            ${reg ? `<input type="text" id="name-input" class="auhtorization-form_input" placeholder="Введите имя" />` : ``}
                            ...
                            <button class="authorization-form_button" id="login-button">${reg ? `Зарегистрироваться` : `Войти`}</button>
                            <a href="#" class="authorization-form_link" id="switch-link">${reg ? `Войти` : `Зарегистрироваться`}</a>
                        </div>
                     </div>`;
